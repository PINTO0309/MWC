...
from .model import MWC, ModelConfig

LOGGER = logging.getLogger("mwc")

LABEL_MAP = {0: "no_masked", 1: "masked"}
                ...
                results.append(
                    {
                        ...
                        "prob_masked": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    train_dataset = MWCDataset(train_samples, transform=train_transform)
    val_dataset = MWCDataset(splits["val"], transform=eval_transform) if splits["val"] else None
    test_dataset = MWCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = MWC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"mwc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "mwc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"mwc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "mwc_best_", 10)
...
def _gather_image_paths(inputs: Sequence[str]) -> List[Path]:
    ...
    for item in inputs:
        ...
        if path.is_file():
            ...
            paths.extend(sorted(p for p in path.rglob("*") if p.is_file() and p.suffix.lower() in allowed_exts))
        else:
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_masked"],
        dynamic_axes={"images": {0: "batch"}, "prob_masked": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MWC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the MWC binary classifier from parquet data.")
