...
class Sample:
    """Lightweight container describing a single mask-classification image."""
...
POSITIVE_LABELS = {"masked", "mask", "with_mask"}
NEGATIVE_LABELS = {"no_masked", "no_mask", "without_mask", "unmasked"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "masked" if label == 1 else "no_masked"))
        source = str(row.get("source", "unknown") or "unknown")
        base_frame = Path(display_path).name
        video_id = str(row.get("video_id", "") or "").strip()
        video_group = video_id or source or Path(display_path).parent.name
...
def split_samples(
    ...
    """Group samples by split labels, with ratio-based fallback for train-only datasets."""
    ...
    has_explicit_eval_split = bool(assignments["val"] or assignments["test"])
    if has_explicit_eval_split:
        logger.info(
            "Using pre-defined dataset splits from parquet (train=%d, val=%d, test=%d).",
            len(assignments["train"]),
            len(assignments["val"]),
            len(assignments["test"]),
        )
    elif assignments["train"] and (val_ratio > 0.0 or test_ratio > 0.0):
        logger.info(
            "Parquet only contains 'train' rows; deriving splits with ratios train=%.3f, val=%.3f, test=%.3f.",
            train_ratio,
            val_ratio,
            test_ratio,
        )
        assignments = _split_train_only_samples(assignments["train"], train_ratio, val_ratio, test_ratio, seed)
    else:
        logger.info(
            "Using all %d parquet rows for training because no validation/test split was provided.",
            len(assignments["train"]),
        )
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'masked').",
            split,
            total,
...
class MWCDataset(Dataset):
    """Torch dataset that reads mask-classification images from disk or embedded bytes."""
...
def _split_train_only_samples(
    ...
    train_share = float(train_ratio) / total_ratio
    val_share = float(val_ratio) / total_ratio
    test_share = float(test_ratio) / total_ratio
    ...
    assignments: Dict[str, List[Sample]] = {split: [] for split in ("train", "val", "test")}
    for label_samples in buckets.values():
        shuffled = list(label_samples)
        rng.shuffle(shuffled)
        total = len(shuffled)
        val_count = int(total * val_share)
        test_count = int(total * test_share)
        train_count = total - val_count - test_count
        if train_count <= 0:
            train_count = max(1, total - test_count)
            val_count = max(0, total - train_count - test_count)
        assignments["train"].extend(shuffled[:train_count])
        assignments["val"].extend(shuffled[train_count : train_count + val_count])
        assignments["test"].extend(shuffled[train_count + val_count : train_count + val_count + test_count])

    for split in assignments:
        assignments[split].sort(key=lambda sample: sample.index)
    return assignments
